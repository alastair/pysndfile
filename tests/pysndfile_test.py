...
def test_compressed(input_file, format, encoding, level, lossy):
    if format in majors:
        if encoding in get_sndfile_encodings(format):
            ...
            if level >= 0:
                compression_command = pysndfile.sndio.Commands(SFC_SET_COMPRESSION_LEVEL=level)
            else:
                compression_command = None
            pysndfile.sndio.write(os.path.join(mydir,'testsndio.{0}_{1}'.format(format, encoding)), ss, rate=sr, format=format, enc=encoding, commands=compression_command)
